...
abstract class GuardedActor extends Actor with Stash {
  ...
  private sealed trait GuardResult[+A] {
    def transformResult[B](ifTransition: Guard[A] => Guard[B],
      ifComplete: A => GuardResult[B]): GuardResult[B] = this match {
      case Pass => Pass
  ...
  trait Guard[+A] {
    ...
    def &[B](other: Guard[B]): Guard[A & B] = new Guard[A & B] {
      ...
      private class Partial[X](remaining: Guard[X])(result: X => A & B) extends Guard[A & B] {
        protected[GuardedActor] def >>:(
          msg: Any) = msg >>: remaining transformResult(new Partial(_)(result), result andThen Complete.apply)
      }
    ...
    def filter(f: A => Boolean): Guard[A] = new Guard[A] {
      protected[GuardedActor] def >>:(msg: Any) = msg >>: self transformResult(_ filter f,
        a => if (f(a)) Complete(a) else Pass)
    }

    def map[B](f: A => B): Guard[B] = new Guard[B] {
      protected[GuardedActor] def >>:(
        msg: Any) = msg >>: self transformResult(_ map f, f andThen Complete.apply)
    }

    def flatMap[B](f: A => Guard[B]): Guard[B] = new Guard[B] {
      protected[GuardedActor] def >>:(
        msg: Any) = msg >>: self transformResult(_ flatMap f, f andThen Restart.apply)
    }
  }
  ...
  def getType[A](implicit tag: reflect.ClassTag[A]): Guard[A] = new Guard[A] {
    protected[GuardedActor] def >>:(
      msg: Any) = if (tag.runtimeClass.isInstance(msg)) Complete(msg.asInstanceOf[A]) else Pass
  }
  ...
  def receive = receiveGuarded(guard.map(context.become))
