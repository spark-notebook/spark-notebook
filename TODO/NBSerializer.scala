...
object NBSerializer extends Logging {
  ...
  val DATETIME_FORMATTER = DateTimeFormatter.ISO_ZONED_DATE_TIME

  def parseDateTime(str: String): Date = Date.from(ZonedDateTime.parse(str, DATETIME_FORMATTER).toInstant)

  def formatDateTime(dt: Date): String = DATETIME_FORMATTER.format(dt.toInstant)
